...
def show_graph(x, y, width, height, title, m_pred=None, b_pred=None):
    ...
    plt.title(title)
    plt.xlabel(i18n.t('Fluid'))
    plt.ylabel(i18n.t('Temperature'))
    plt.xlim([-1, 11])
    plt.ylim([-2, 11])
...
def show_metrics(m, b, cost):
    ...
    col1.metric(i18n.t('Slope'), round(m, 3))
    col2.metric(i18n.t('Intercept'), round(b, 3))
    col3.metric(i18n.t('Cost'), round(cost, 3))
...
show_graph(x, y, WIDTH_FIG, HEIGHT_FIG, i18n.t('title_graph_1'))

st.markdown(i18n.t('comment_1_graph_1'))
st.markdown(i18n.t('comment_2_graph_1'))
st.markdown(i18n.t('comment_3_graph_1'))
st.markdown(i18n.t('comment_4_graph_1'))
st.markdown("\n")

# 'm' and 'b' random initial values for applying SGD method
st.markdown(i18n.t('comment_initial_values'))
m_init = 0.9
b_init = 0.1
...
show_graph(x, y, WIDTH_FIG, HEIGHT_FIG, i18n.t('title_graph_2'), m_init, b_init)

# SGD method
m_pred_v, b_pred_v, Cost_v = sgd(x, y, m_init, b_init, EPOCHS)

st.markdown(i18n.t('comment_SGD'))
...
show_graph(x, y, WIDTH_FIG, HEIGHT_FIG, i18n.t('title_graph_3'), m_epoch, b_epoch)
